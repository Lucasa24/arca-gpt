...
const { toNum, calcularSaldosCreditos, calcularResumoUso } = require("./creditsMetrics.js");
...
function withUserLock(userId, task) {
  ...
  const prev = USER_LOCKS.get(key) || Promise.resolve();
  let next = prev.then(task, task);
  next = next.finally(() => {
    if (USER_LOCKS.get(key) === next) USER_LOCKS.delete(key);
  });
  USER_LOCKS.set(key, next);
  return next;
}
...
async function debitarCreditos(userId, valor, dados = {}, authHeader = null) {
  ...
  // Idempotência: se o mesmo consumption_id reaparecer (retry/reprocessamento), ignorar para não duplicar histórico.
  const consumptionId = dados && dados.consumption_id ? String(dados.consumption_id) : null;
  if (consumptionId) {
    const list = Array.isArray(rec.historico_de_consumo) ? rec.historico_de_consumo : [];
    const seen = list.slice(0, 80).some((x) => x && String(x.consumption_id || "") === consumptionId);
    if (seen) return { ok: true, debitado: 0, record: rec, idempotente: true };
  }
...
async function registrarConsumo(userId, dados = {}, authHeader = null) {
  ...
  if (consumptionId) {
    const list = Array.isArray(rec.historico_de_consumo) ? rec.historico_de_consumo : [];
    const seen = list.slice(0, 80).some((x) => x && String(x.consumption_id || "") === consumptionId);
    if (seen) return { ok: true, record: rec, idempotente: true };
  }
...
async function gerarComCreditos({ userId, prompt, authHeader = null, gemini = {} }) {
  ...
  const bucket5m = Math.floor(Date.now() / 300000);
  const rawKey = `${uid}|${String(prompt || "").trim()}|${bucket5m}`;
  const digest = crypto.createHash("sha256").update(rawKey).digest("hex").slice(0, 14);
  const consumoBaseId = `gem_${digest}`;
  ...
  if (custo > 0) {
    ...
    const ratioFree = freeDebit > 0 ? freeDebit / custo : 0;
    const tokensFree = Math.min(tokens, Math.max(0, Math.round(tokens * ratioFree)));
    const tokensPaid = Math.max(0, tokens - tokensFree);
    if (freeDebit > 0) {
      await debitarCreditos(uid, freeDebit, {
        ...
        custoCreditos: freeDebit,
        consumption_id: `${consumoBaseId}:free`
      }, authHeader);
      restante -= freeDebit;
    }
    if (restante > 0) {
      await debitarCreditos(uid, restante, {
        ...
        cache: !!r.cache,
        prompt_len: String(prompt || "").length,
        custoCreditos: restante,
        consumption_id: `${consumoBaseId}:paid`
      }, authHeader);
    }
  } else if (tokens > 0) {
    const bucket = saldosAntes.free > 0 ? "free" : "paid";
    await registrarConsumo(uid, {
      tipo: "gemini",
      bucket,
      modelo: r.modelo,
      tokens,
      ...
      prompt_len: String(prompt || "").length,
      custoCreditos: 0,
      consumption_id: `${consumoBaseId}:${bucket}`
    }, authHeader);
  }
  ...
  const resumoUso = updated ? calcularResumoUso(updated) : calcularResumoUso(rec);
  return {
    ok: true,
    mensagem: r.mensagem,
    modelo: r.modelo,
