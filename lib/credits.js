...
function sumTokens(list, pred) {
  return (Array.isArray(list) ? list : []).reduce((acc, it) => {
    if (!it || (pred && !pred(it))) return acc;
    return acc + Math.max(0, Math.floor(toNum(it.tokens)));
  }, 0);
}
...
function calcularResumoUso(rec, nowMs = Date.now()) {
  ...
  return {
    todayFreeTokensUsed,
    todayFreeTokensLimit: FREE_DAILY_TOKENS,
    sinceRechargePaidCreditsSpent: Math.max(0, Math.floor(toNum(sinceRechargePaidCreditsSpent))),
    totalTokensUsed
  };
}
