...
const geminiApi = require("../api/gemini.js");
const gerarMensagemGemini = geminiApi.gerarMensagemGemini;
const gerarMensagemGeminiStream = typeof geminiApi.gerarMensagemGeminiStream === "function"
  ? geminiApi.gerarMensagemGeminiStream
  : null;
const { addGeminiUsage, shouldBlockFree, markFreeExhausted } = require("./geminiQuota.js");
...
const FREE_CREDITS = Math.max(
  0,
  Math.floor(Number(process.env.GEMINI_FREE_CREDITS || process.env.ARCA_GEMINI_FREE_CREDITS || 0))
);
...
async function gerarComCreditos({ userId, prompt, authHeader = null, gemini = {} }) {
  ...
  const rec = await getOrCreateCredits(uid, authHeader);
  const saldosAntes = rec ? calcularSaldosCreditos(rec) : { total: 0, free: 0, paid: 0 };

  const quotaCheck = await shouldBlockFree(uid);
  const freeBlocked = !!(quotaCheck && quotaCheck.blocked);
  const canUsePaid = saldosAntes.total > 0;

  if (freeBlocked && !canUsePaid) {
    return {
      ok: false,
      tipo: "limite_gratis",
      blocked: true,
      reason: "Limite grátis da Arca atingido. Tente novamente mais tarde."
    };
  }
  ...
  if (!r.ok) {
    const isLimit = r && (r.status === 429 || r.tipo === "limite_gemini");
    if (isLimit) {
      await markFreeExhausted(uid);
      return {
        ok: false,
        tipo: "limite_gratis",
        blocked: true,
...
async function gerarComCreditosStream({ userId, prompt, authHeader = null, gemini = {}, onChunk }) {
  const uid = normalizeUserId(userId);
  if (!uid) return { ok: false, tipo: "sem_credito" };
  ...
  let r;
  if (typeof gerarMensagemGeminiStream === "function") {
    r = await gerarMensagemGeminiStream(prompt, {
      contents: gemini.contents,
      systemInstruction: gemini.systemInstruction,
      cacheKey: gemini.cacheKey,
      onChunk
    ...
    r = await gerarMensagemGemini(prompt, {
      contents: gemini.contents,
      systemInstruction: gemini.systemInstruction,
      cacheKey: gemini.cacheKey
    });
    if (r && r.ok && typeof onChunk === "function") {
      try {
        onChunk(String(r.mensagem || ""));
      } catch {}
    }
  }
