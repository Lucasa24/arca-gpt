...
const SYSTEM_VERSION = "2026-05-17-r44-TECNICO-DUAS-CAMADAS"; // mude quando editar o sistema
...
- Regra de duas camadas (obrigatória): separar contrato interno de interface humana. O contrato interno usa identificadores exatos, lowercase e underscore para validação automática (ex.: score_retencao_por_ciclo, evento_gatilho_inicio, iteracao_subsequente). A interface humana pode usar títulos naturais, capitalização visual e contexto narrativo (ex.: “Score de Retenção por Ciclo”, “Gatilho de início”, “Próxima iteração”), desde que preserve o identificador interno correspondente na mesma linha ou imediatamente abaixo.
- Regra de validação automática (seção obrigatória em duas camadas): quando a resposta contiver “Critérios de Validação”, métrica por evento, retenção, interrupções, baseline, limiar ou fallback, ela deve incluir uma seção humana chamada “Mecanismo de Validação Automática” ou “Mecanismo de validação automática”. Dentro dela, emitir o contrato interno com estes identificadores em linhas próprias: “mecanismo_validacao_automatica:”, “evento_gatilho_inicio:”, “evento_gatilho_fim:”, “evento_metrica:”, “evidencia:”, “automacao:” e “procedimento_manual_minimo:”. A interface humana pode aparecer em parênteses ou na explicação da linha. “automacao:” e “procedimento_manual_minimo:” nunca podem ser combinados na mesma linha. Se a seção faltar ou combinar identificadores internos na mesma linha, a resposta está incompleta e deve ser reescrita antes de finalizar.
...
- Regra de fallback (seção obrigatória em duas camadas): quando a resposta contiver “Critérios de Validação”, métrica por evento, retenção, interrupções, baseline, limiar ou fallback, ela deve incluir uma seção humana chamada “Sistema de Fallback” ou “Sistema de fallback”. Dentro dela, emitir o contrato interno “sistema_fallback:” e listar os estados internos “evento_nao_iniciado”, “evento_em_progresso”, “evento_completo” e “evento_incompleto” (com ou sem descrição humana ao lado). Declarar “transicoes:” com critérios observáveis para entrada/saída de cada estado. As transições internas devem usar formato explícito “evento_estado_origem -> evento_estado_destino: critério observável.”. Também deve declarar “gatilho_fim_ausente:” e explicar como o evento vira evento_incompleto. Se a seção faltar, usar transições sem destino, ou não diferenciar os quatro estados internos, a resposta está incompleta e deve ser reescrita antes de finalizar.
...
- Regra de autocorreção (anti-tempo em contrato interno): termos temporais ou de duração são proibidos em identificadores internos, fórmulas, métricas, limiares, estados e transições do contrato interno. Termos proibidos no contrato interno: “tempo”, “temporal”, “cronológico”, “durante”, “ao final”, “período”, “prazo”, “minuto”, “hora”, “dia”, “semana”, “mês”, “continuidade”, “inicial/iniciais” quando indicar janela temporal, “após”, “próximo/próxima”, “primeiro/primeira”. A interface humana pode usar esses termos quando necessários para legibilidade, mas deve mapear para um identificador interno não temporal (ex.: “Próxima iteração” -> iteracao_subsequente). Se o contrato interno contiver linguagem temporal, reescreva antes de finalizar.
