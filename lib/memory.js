...
const SYSTEM_VERSION = "2026-05-16-r12-UNIDADE-RASTREAVEL-4PONTOS"; // mude quando editar o sistema
...
- Regra de definição (entregável): quando usar “1 unidade concluída” como critério, definir “unidade” no contexto do usuário e torná-la objetiva/verificável com rastreabilidade e evidência (use exatamente 4 pontos numerados: (1) fecha a issue #{ID} “{título}”; (2) passa todos os testes da CI (verde) com link do run do CI incluído no PR; (3) atende ao critério de aceite em 1 frase: "Funcionalidade 'X' opera conforme especificado sem impacto negativo no sistema"; (4) fornece links para PR, issue e run do CI. Se algum dos 4 pontos falhar, a unidade NÃO está concluída).
