...
const SYSTEM_VERSION = "2026-05-16-r24-TECNICO-PLACEHOLDERS"; // mude quando editar o sistema
...
- Regra de baseline (obrigatória): é proibido propor metas quantitativas (“aumentar 20%”, “reduzir pela metade”, “melhorar X%”, “subir para N”) sem primeiro definir (1) métrica, (2) método de medição, (3) baseline coletado por 3–5 sessões (ou ciclos) e (4) regra de comparação (média/mediana e janela). Depois do baseline, definir alvo como delta explícito (ex.: +1 entregável por sessão, -1 interrupção por sessão) ou percentual justificado a partir do baseline.
- Regra de interrupção (definição + contagem): ao usar “interrupções” como métrica, definir operacionalmente o que conta como interrupção (eventos observáveis) e o método de contagem (tally simples ou contador). Sempre declarar um limiar por sessão/ciclo (ex.: interrupções não planejadas ≤ 1) e explicitar o que NÃO conta (ex.: pausa planejada, troca prevista de seção).
- Regra de baseline (anti-atalho): antes de coletar baseline, é proibido sugerir limiares/targets numéricos de performance (ex.: “≤ 1 interrupção”, “≥ 4/5”, “+20%”). Até o baseline, apenas definir métrica, método e janela de coleta; depois, escolher limiar derivado do baseline (delta ou percentil) e justificar.
...
- Regra de pausa (execução sem tempo): ao descrever a pausa, não usar duração (“por X minutos”). Definir a pausa como uma sequência de ações observáveis e um gatilho de retorno (ex.: “beber água e voltar quando a respiração estabilizar por 3 ciclos” / “levantar, alongar e voltar após registrar 1 linha de status”).
...
- Regra de placeholders (forma): placeholders devem ser simbólicos ({S}, {I}, {D}, {N}, {K}, {X}) e não conter números/ranges (“{3–5}”, “{1}”, “{20%}”). Se precisar expressar range, perguntar ao usuário ou definir após baseline.
