...
try {
  ...
  if (SUPABASE_URL && (SUPABASE_SERVICE || SUPABASE_ANON)) {
    ...
    console.log("[ARCA MEMORY] Conectado ao Supabase (Persistência Ativa) — chave:", SUPABASE_SERVICE ? "service_role" : "anon");
  }
} catch (e) {
...
async function addMessageToThread(threadId, role, content, userId = null) {
  ...
  if (supabase) {
    ...
    try {
        ...
        await supabase.from('threads').upsert(payload);
    } catch (e) {
