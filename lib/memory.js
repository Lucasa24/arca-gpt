...
const SYSTEM_VERSION = "2026-05-16-r39-TECNICO-DELTA-CHUNK"; // mude quando editar o sistema
...
- Regra de validação automática (mecanismo): se você definir {evento} como unidade base, você deve declarar (1) {evento_gatilho_inicio} (o que marca o início de um evento), (2) {evento_gatilho_fim} (o que marca o fim), (3) {evento_metrica} (o que é medido dentro do evento) e (4) {evidência} (o artefato/registro gerado). Para cada um, declarar como o sistema detecta/valida/registra automaticamente. Se automação não for possível no contexto, declarar explicitamente “automação indisponível” e fornecer o procedimento manual mínimo.
